...
# Keep track of guilds where welcome message was sent
sent_welcome = set()
...
class NumberSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        # For now, do nothing on selection
...
async def on_guild_join(guild):
    ...
    # Try to send in channel named 'general'
    ...
    if channel and channel.permissions_for(guild.me).send_messages:
        ...
        # Fallback: first channel where bot can send messages
        for channel in guild.text_channels:
            if channel.permissions_for(guild.me).send_messages:
                await channel.send(embed=embed, view=NumberDropdown())
                break

# Optional: test command to manually send the welcome message
@bot.command()
